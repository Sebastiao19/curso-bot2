...
driver = webdriver.Chrome('/home/tiao/Documentos/curso_python_bot/chromedriver', options=options)
...
while pagina_atual <= 10:
    ...
    divs = driver.find_element_by_xpath("//div[@class='g']")
    for div in divs:
...
with open("resutadosrobo.txt","w") as arquivo:
    for resultado in lista_resultados:
        arquivo.write("%s\n" %resultado)
    arquivo.close

print("%s resultados encontrados do ggogle e salvos no arquivo resultados.txt" % len(lista_resultados))